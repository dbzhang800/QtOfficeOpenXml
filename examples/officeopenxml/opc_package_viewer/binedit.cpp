...
void BinEdit::setData(const QByteArray &data)
{
    setSizes(0, data.size());

    int block = 0;
    while (true) {
        QByteArray d = data.mid(block*m_blockSize, m_blockSize);
        if (d.isEmpty())
            break;
        if (d.size() != m_blockSize)
            d += QByteArray(m_blockSize - d.size(), 0);
        addData(block, d);
        block++;
    }
}
...
void BinEdit::setSizes(quint64 startAddr, int range, int blockSize)
{
    ...
    Q_ASSERT((blockSize/m_bytesPerLine) * m_bytesPerLine == blockSize);
...
void BinEdit::paintEvent(QPaintEvent *e)
{
    ...
    Q_ASSERT((size_t)m_bytesPerLine < sizeof(changedString));
    const char *hex = "0123456789abcdef";
...
void BinEdit::asFloat(int offset, float &value, bool old) const
{
    ...
    Q_ASSERT(data.size() ==  sizeof(float));
...
void BinEdit::asDouble(int offset, double &value, bool old) const
{
    ...
    Q_ASSERT(data.size() ==  sizeof(double));
